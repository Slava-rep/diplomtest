...
# diplomtest/employees/models.py
from django.db import models
from django.conf import settings
...
class EmployeesEmployee(models.Model):
    ...
    snils = models.CharField(unique=True, max_length=14)
...
class EmployeesEmployeemeasurementtype(models.Model):
    id = models.BigAutoField(primary_key=True)
    employee = models.ForeignKey(EmployeesEmployee, models.DO_NOTHING)
    measurement_type_id = models.BigIntegerField()
    
    class Meta:
        managed = False
        db_table = 'employees_employeemeasurementtype'
        unique_together = (('employee', 'measurement_type_id'),)
        verbose_name = "Вид измерений сотрудника"
        verbose_name_plural = "Виды измерений сотрудников"
    
    def __str__(self):
        return f"{self.employee} - {self.measurement_type_id}"