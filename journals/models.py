...
# diplomtest/journals/models.py
from django.db import models

class JournalsJournalregistration(models.Model):
    id = models.BigAutoField(primary_key=True)
    receipt_date = models.DateField()
    receipt_signature = models.CharField(max_length=100)
    issue_date = models.DateField(blank=True, null=True)
    issue_signature = models.CharField(max_length=100, blank=True, null=True)
    ...
    def __str__(self):
        return f"{self.si} - {self.receipt_date}"

class JournalsJournalverification(models.Model):
    id = models.BigAutoField(primary_key=True)
    verification_result = models.CharField(max_length=100)
    certificate = models.ForeignKey('certificates.CertificatesCertificate', models.DO_NOTHING)
    si = models.ForeignKey('si.SiSi', models.DO_NOTHING)
    ...
    def __str__(self):
        return f"{self.si} - {self.verification_result}"

class JournalsJournalenvironment(models.Model):
    id = models.BigAutoField(primary_key=True)
    measurement_date = models.DateField()
    affecting_factors = models.ForeignKey('si.SiAffectingfactors', models.DO_NOTHING)
    ...
    def __str__(self):
        return f"Условия среды от {self.measurement_date}"